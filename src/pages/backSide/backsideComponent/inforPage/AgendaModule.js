...
const AgendaModule = ({ theAgenda }) => {
    ...
    const { getOneAgenda, agenda } = useAppContext()
    ...
    const url = agenda.file
    return (
        <div className='flex-row ac-center ai-center jc-center ji-center border-b-1-grey' >
            <div className='m-r-30px' >{theAgenda.date}</div>
            <div className='margin-l-10px' onClick={onClickHandler} >click here to see detail</div>
        </div>
    )
}
